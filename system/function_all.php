...
function getTheme(){
	global $mysqli, $setting, $data;
	$t = isset($setting['default_theme']) ? trim($setting['default_theme']) : '';
	if($t == '' || !file_exists(BOOM_PATH . '/css/themes/' . $t . '/' . $t . '.css')){
		$fallBack = array('Lite', 'Dark', 'Strawberry', 'Cody');
		foreach($fallBack as $oneTheme){
			if(file_exists(BOOM_PATH . '/css/themes/' . $oneTheme . '/' . $oneTheme . '.css')){
				$t = $oneTheme;
				break;
			}
		}
...
function getPrivateCount($user){
	global $mysqli;
	$count = $mysqli->query("SELECT count(*) as pcount FROM boom_conversation WHERE target = '{$user['user_id']}' AND unread > 0 ");
	if($count->num_rows > 0){
		$result = $count->fetch_assoc();
		return (int) $result['pcount'];
	}
	return 0;
}

function profileStyle($user){ return false; }
function styleDetails($id){ return array(); }
function canProfileStyle(){ return false; }
function userShareRelation($user){
        if(isset($user['user_relation']) && $user['user_relation'] > 0){
                return true;
        }
        return false;
}
function canProfileMusic(){
        global $setting, $data;
        if(isset($setting['allow_pmusic']) && boomAllow($setting['allow_pmusic']) && !featureBlock($data['bupload'])){
                return true;
        }
        return false;
}
